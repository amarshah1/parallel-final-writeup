...
with \texttt{jemalloc} and used \texttt{numactl --interleave all} to spread
...
are shown in Figure~\ref{fig:random}: As we can see, the speedup over
...
It takes the median percentage of time per round from all of the benchmarks. We
measure three rounds: (a) frontier is the parallel \texttt{filter} to gather the
frontier of changed classes, (b) semisort is the parallel semisort to group
parent e-nodes by their current, and (c) consolidate is the parallel pass to
union together all pairs of the same class in the semisort output. We notice
that the semisort is the most expensive phase, taking around 25.4\%, 22.1\%, and
19.2\% of the time in rounds $1$, $2$, and $3$ respectively. This suggests that
optimizing the semisort phase could lead to performance improvements. We discuss
our optimizations of semisort in \autoref{sec:impl}.
...
We plot our speedups in \autoref{fig:egg:loglog} and the per-round merge counts
in \autoref{fig:egg:rounds}.
We obtain speedups up to $5.69\times$ on 8 cores, $8.99\times$ on 16 cores,
$12.0\times$ on 32 cores, and $8.70$ on 128 cores for the Herbie benchmarks. The
speedup is not as high as on the synthetic benchmarks, likely because the Herbie
benchmarks have fewer merges per round and more rounds of congruence closure,
which leads to higher overheads in our parallel algorithm. Note that the very small
benchmarks in \autoref{fig:egg:rounds} are the ones in which the parallel algorithm 
actually slows down on.

However, we still see some performance improvement over the sequential version,
especially up to $16$ cores, demonstrating the potential of our parallel
congruence closure algorithm in real-world applications.

While this speedup is impressive, it comes with the caveat that the each of
these congruence closure benchmarks are relatively simple (all of them can be
solved in less than $3$ seconds sequentially). We picked these benchmarks
because they were used to evaluate congruence closure
previously~\cite{shortproofs}. Future work should try to evaluate our algorithm
on more challenging benchmarks such as the equality saturation based optimizing
compiler \texttt{egg-cc}~\cite{eggcc}. We believe our algorithm would perform
well on these benchmarks, providing a meaningful speedup on ``real world''
benchmarks.
